...
> **주의**: `lab.ssafy.com`은 포트 22가 외부에서 차단되어 SSH git 접속이 불가합니다.
> HTTPS + Personal Access Token 방식을 사용합니다.

### Personal Access Token 발급

1. `lab.ssafy.com` 로그인
2. 오른쪽 위 아바타 → **Preferences → Access Tokens**
3. Token name: `ec2-deploy`, Scopes: `read_repository` + `write_repository` 체크
4. **Create personal access token** → 토큰 복사 (한 번만 표시됨)
...
### GitLab Token Credentials (코드 pull용)

1. Jenkins 관리 → Credentials → Global → **Add Credentials**
2. Kind: `Username with password`
3. ID: `gitlab-token`
4. Username: GitLab 아이디
5. Password: 6단계에서 발급한 Personal Access Token
6. Save
